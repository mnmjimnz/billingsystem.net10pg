...
# 1. UPDATE REPORTS.HTML
with open('Frontend/pages/reports.html', 'r', encoding='utf-8') as f:
    html = f.read()

# Replace Styles
old_styles = r'''        /\* Estilos Empresariales Específicos para Reportes \*/
...
new_styles = '''        /* Estilos Formales de Reportes Empresariales */
...
html = re.sub(old_styles, new_styles, html)
...
# Replace tables classes
html = html.replace('class="table table-sm align-middle report-table"', 'class="formal-table"')

# Add document headers to tables
header_template = lambda title, prefix: f'''                                <div class="d-flex justify-content-between align-items-start mb-4">
                                    <div style="flex: 1;">
                                    ...
                                    <div class="ms-4">
                                        <button class="btn btn-sm btn-outline-danger mb-2 w-100" onclick="exportPDF('{prefix}Table', 'Reporte_{title.replace(' ', '_')}')"><i class="bi bi-file-pdf"></i> PDF</button>
                                        <button class="btn btn-sm btn-outline-success w-100" onclick="exportExcel('{prefix}Table', 'Reporte_{title.replace(' ', '_')}')"><i class="bi bi-file-excel"></i> Excel</button>
                                    </div>
                                </div>'''

def replace_header(html_text, old_h5, title, prefix):
    regex = r'<div class="d-flex justify-content-between align-items-center mb-3">\s*<h5 class="fw-bold text-dark mb-0">' + old_h5 + r'</h5>\s*<div>[\s\S]*?</div>\s*</div>'
    return re.sub(regex, header_template(title, prefix), html_text)

html = replace_header(html, "Detalle de Ventas Registradas", "Reporte General de Ventas", "sales")
html = replace_header(html, "Detalle de Adquisiciones \(Compras\)", "Reporte General de Compras", "purchases")
html = replace_header(html, "Movimientos de Inventario \(Kardex\)", "Movimientos de Inventario (Kardex)", "kardex")
html = replace_header(html, "Actividad y Cortes de Caja por Usuario", "Actividad de Usuarios y Turnos", "activity")
html = replace_header(html, "Estado de Flujos de Efectivo", "Estado de Flujos de Efectivo", "cashflow")

with open('Frontend/pages/reports.html', 'w', encoding='utf-8') as f:
    f.write(html)

# 2. UPDATE REPORTS.JS
with open('Frontend/pages/reports.js', 'r', encoding='utf-8') as f:
    js = f.read()
...
'''
js = js.replace('/* ================= VENTAS ================= */', meta_updater + '\n/* ================= VENTAS ================= */')
...
with open('Frontend/pages/reports.js', 'w', encoding='utf-8') as f:
    f.write(js)
